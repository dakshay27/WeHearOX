...
public class ApiTest {
    ...
    public ApiTest(String host, String apiKey, String sessionId) {
        this.host = host;
        this.apiKey = apiKey;
        this.sessionId = sessionId;
    }
    public static synchronized ApiTest getInstance(String host, String apiKey) {
        if (instance == null) {
            instance = new ApiTest(host,apiKey);
        }
        return instance;
    }
    public static synchronized ApiTest getInstance(String host, String apiKey,String sessionId) {
        if (instance == null) {
            instance = new ApiTest(host,apiKey,sessionId);
        }
        return instance;
    }
    /**
    ...
    public void authenticate(final String username, final String password, final ResultCompletionHandler completionHandler) {
        callAPI("user/gettoken", HTTPMethod.GET, null, new ResultCompletionHandler() {
            ...
            public void onResult(boolean success, JSONObject response, Exception exception) {
                // On main thread.
                if (!success) {
                    Log.d(TAG,"Authenatice1:"+success);
                    completionHandler.onResult(false, response, exception);
                    return;
                }

                if (response == null) {
                    Log.d(TAG,"Authenatice2:"+success);
                ...
                try {
                    ...
                    if (apiKey == null) {
                        Log.d(TAG,"Authenatice3:"+success);
                        completionHandler.onResult(false, response, exception);
                        return;
                    }
                    ...
                    Log.d(TAG,"Authenatice4:"+success);
